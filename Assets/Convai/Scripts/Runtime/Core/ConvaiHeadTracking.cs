...
namespace Convai.Scripts.Runtime.Core
{
    ...
    public class ConvaiHeadTracking : MonoBehaviour
    {
        ...
        private float _actionTransitionSpeed = 2f; 
        ...
        private void PerformHeadTracking()
        {
            if (_isActionRunning)
            {
                // Aksiyon çalışırken kafa pozisyonunu yavaşça sıfırla
                _headPivot.localRotation = Quaternion.Slerp(_headPivot.localRotation, Quaternion.identity, Time.deltaTime * _actionTransitionSpeed);
                _currentLookAtWeight = Mathf.Lerp(_currentLookAtWeight, 0, Time.deltaTime * _actionTransitionSpeed);
                
                // Eğer neredeyse sıfıra ulaştıysak, tam olarak sıfırla
                if (Quaternion.Angle(_headPivot.localRotation, Quaternion.identity) < 0.1f)
                {
                    _headPivot.localRotation = Quaternion.identity;
                    _currentLookAtWeight = 0;
                }
        ...
        {
            if (_isActionRunning == newValue) return; // Eğer durum değişmediyse, bir şey yapma

            _isActionRunning = newValue;

            // Eğer önceki geçiş hala çalışıyorsa, onu durdur
            ...
            // Yeni geçişi başlat
            _smoothTransitionCoroutine = StartCoroutine(SmoothTransition(newValue));
        }
        
        private IEnumerator SmoothTransition(bool toActionRunning)
        {
            float elapsedTime = 0f;
            float transitionDuration = 0.5f; // Geçiş süresi, isteğe bağlı olarak ayarlanabilir

            Quaternion startRotation = _headPivot.localRotation;
            float startWeight = _currentLookAtWeight;
            ...
            while (elapsedTime < transitionDuration)
            {
                ...
                if (toActionRunning)
                {
                    // Aksiyona geçerken kafayı yavaşça sıfır pozisyonuna getir
                    _headPivot.localRotation = Quaternion.Slerp(startRotation, Quaternion.identity, t);
                    _currentLookAtWeight = Mathf.Lerp(startWeight, 0, t);
                }
                else
                {
                    // Aksiyondan çıkarken, normal takip davranışına geri dön
                    // Burada spesifik bir hedef rotasyon belirtmiyoruz çünkü
                    // normal PerformHeadTracking() fonksiyonu bunu ele alacak
                    _currentLookAtWeight = Mathf.Lerp(startWeight, 1, t);
                }
